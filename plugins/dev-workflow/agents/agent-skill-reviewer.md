...
description: Use to review the definition file of a Claude Code subagent (`.claude/agents/<name>.md`) or a skill (`SKILL.md`) and return a report of incoherences and possible improvements. Checks the frontmatter, the all-important `description`/trigger field, internal coherence (does the body deliver what the description promises, are declared tools actually used), and — corpus-aware — flags overlap, ambiguous routing, and name collisions with sibling agents/skills. Returns a prioritized report with section/line refs — does NOT edit the file. Use when the user asks to "review this agent/skill", "audit my skill definition", or before shipping a new agent/skill. NOT a code reviewer (that's code-reviewer) — this reviews prompt/definition files, not application code.
